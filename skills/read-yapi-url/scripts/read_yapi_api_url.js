 ...
 * Usage: node read_yapi_api_url.js <url> <token> [baseURL]
...
if (!url || !token) {
  ...
  console.error('Usage: node read_yapi_api_url.js <url> <token> [baseURL]');
  process.exit(1);
}
...
const apiUrl = `https://${baseURL}/api/interface/get?token=${token}&id=${apiId}`;
